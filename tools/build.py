...
def build():
    ...
    body = (breadcrumbs([("/", "Главная"), (None, "Электронные книги")])
            ...
            + '<p class="center">Электронные версии книг доступны для чтения и '
              'скачивания.</p>'
            + (f'<p class="center"><a class="btn" href="{esc(drive)}" '
               'target="_blank" rel="noopener">Открыть книги →</a></p>' if drive else ""))
